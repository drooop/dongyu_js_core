...
4. `docs/WORKFLOW.md`
5. `docs/ITERATIONS.md`
6. `docs/_templates/iteration_plan.template.md`
7. `docs/_templates/iteration_resolution.template.md`
8. `docs/_templates/iteration_runlog.template.md`
9. `docs/ai-work-conventions.md` (if present)

Then (as needed):
10. `docs/iterations/<ITERATION_ID>/`
11. `scripts/` test entry points and `.env`/`.env_test` conventions (if present)
...
### Charter Compliance Checklist (REQUIRED)
- Explicit references to applicable `docs/charters/*.md` clauses
- Confirm no Charter violations are introduced
- Any uncertainty + verification method
...
Phase 3 may begin ONLY after an **Approved** outcome per `docs/WORKFLOW.md`.

Acceptable approval sources:
- User explicitly says **Approved** / “审核通过” / “审核通过，开始 Phase3”
- OpenCode Auto-Approval after 3 independent reviews (see policy below)

If approval is ambiguous, stop and ask for a clear Phase 2 outcome: Approved / Change Requested / On Hold.

### OpenCode Auto-Approval Policy (3 reviews)

When the user does not explicitly approve, OpenCode may substitute the Review Gate by running multiple independent reviews.

Rules:
- Perform exactly 3 independent reviews of `plan.md` + `resolution.md` in the following fixed sequence:
  1) Review #1: `@oracle` — SSOT/Charter alignment and architecture risk review
  2) Review #2: `@momus` — Plan/Resolution completeness, verifiability, and loophole review
  3) Review #3: `@oracle` — Final go/no-go review after applying any fixes
- The latest 3 review decisions must all be **Approved**.
- No unresolved **Change Requested** items remain.
- Record each review as a factual entry in `docs/iterations/<id>/runlog.md` Environment (see `docs/ai-work-conventions.md`).

If all rules pass, OpenCode may treat the gate as Approved and enter Phase 3.
...
### Charter Violation Check (REQUIRED)
- Identify which Charter constraints could be impacted
- Explicitly state whether any violation occurred
- If unclear, stop and document
...
Branch convention (single-founder mode):
- Most iterations: merge `dev_<id>` into `dev` after completion.
- Consider PR only when promoting `dev` → `main`.
