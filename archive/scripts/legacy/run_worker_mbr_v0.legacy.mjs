...
function main() {
  ...
  // MBR function: Matrix → MQTT routing
  // Uses ModelTable route labels (mbr_route_<modelId>) + segment labels for topic construction
  rt.addLabel(sys, 0, 0, 0, {
    ...
    v: [
      ...
      // Read default model ID from ModelTable instead of hardcoding
      "const defaultModelId = ctx.getLabel({ model_id: -10, p: 0, r: 0, c: 0, k: 'mbr_default_model_id' }) || 2;",
      "let targetModelId = defaultModelId;",
      "let pinName = 'patch';",
      ...
      // Route table lookup from ModelTable
      "  if (sourceModelId && Number.isInteger(sourceModelId) && sourceModelId > 0) { targetModelId = sourceModelId; }",
      "  const routeKey = 'mbr_route_' + targetModelId;",
      "  const specificRoute = ctx.getLabel({ model_id: -10, p: 0, r: 0, c: 0, k: routeKey });",
      "  const defaultRoute = ctx.getLabel({ model_id: -10, p: 0, r: 0, c: 0, k: 'mbr_route_default' });",
      "  const route = (specificRoute && typeof specificRoute === 'object') ? specificRoute : ((defaultRoute && typeof defaultRoute === 'object') ? defaultRoute : null);",
      "  if (route && route.pin) { pinName = route.pin; }",
      "  if (pinName === 'event') {",
      "    patch = { version: 'mt.v0', op_id: opId, records: [], action: action, data: data, timestamp: inbox.timestamp };",
      "    console.log('[mbr_mgmt_to_mqtt] ui_event, routing to ' + targetModelId + '/' + pinName);",
      "  } else {",
      ...
      "if (!patch) return;",
      // Build topic from segment labels (9-layer) with fallback to mqtt_topic_base (legacy)
      "const topicMode = String(ctx.getLabel({ model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_mode' }) || '');",
      "let topic = '';",
      "if (topicMode === 'uiput_9layer_v2') {",
      "  const ns = String(ctx.getLabel({ model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_ns' }) || '');",
      "  const ws = String(ctx.getLabel({ model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_ws' }) || 'ws');",
      "  const dam = String(ctx.getLabel({ model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_dam' }) || 'dam');",
      "  const pic = String(ctx.getLabel({ model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_pic' }) || 'pic');",
      "  const de = String(ctx.getLabel({ model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_de' }) || 'de');",
      "  const sw = String(ctx.getLabel({ model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_sw' }) || 'sw');",
      "  if (ns) { topic = ns + '/in/' + ws + '/' + dam + '/' + pic + '/' + de + '/' + sw + '/' + targetModelId + '/' + pinName; }",
      "} else {",
      ...
      "  if (base) { topic = base + '/' + targetModelId + '/' + pinName; }",
      "}",
      "console.log('[mbr_mgmt_to_mqtt] Publishing to MQTT topic:', topic, 'targetModelId:', targetModelId, 'pinName:', pinName);",
      "if (!topic) { console.log('[mbr_mgmt_to_mqtt] ERROR: topic is empty'); return; }",
  ...
  // Build wildcard subscription from segment labels (9-layer) or legacy base
  const topicMode = String(getLabelValue(rt, { model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_mode' }) || '');
  let mqttSubTopic;
  if (topicMode === 'uiput_9layer_v2') {
    const ns = String(getLabelValue(rt, { model_id: 0, p: 0, r: 0, c: 0, k: 'mqtt_topic_ns' }) || 'UIPUT');
    mqttSubTopic = `${ns}/out/#`;
  } else {
  ...
  mqttClient.on('connect', () => {
    ...
    process.stdout.write(`[mbr-worker] mqtt READY subscribed=${mqttSubTopic}\n`);
    process.stdout.write('[mbr-worker] READY\n');
  });

  mqttClient.on('message', (topic, buf) => {
    // Accept all messages matching our wildcard subscription (no hardcoded topic filtering)
    let payload = null;
    try {
