...
```
MQTT 入站消息到达 → runtime 自动写入 PIN 邮箱:
  Cell(model_id, 0, 1, 1) 的 label { k: "event", t: "IN", v: <payload> }
...
⑦ K8s Worker: mqttIncoming()                    ← 你的 DAM Worker 在这里
   ├─ runtime 自动写入 Cell(100, 0, 1, 1) { k:"event", t:"IN", v:<payload> }
   ├─ 事件检测 → 触发 on_model100_event_in
   ├─ 程序模型执行：生成颜色 #38bd7f
...
2. 前端通过 PIN 发送注册命令给 DAM Worker:
   MQTT topic: UIPUT/in/{ws}/{dam}/{pic}/{de}/{sw}/{dam_model}/register
...
3. DAM Worker 建立索引 → 返回确认 patch:
   MQTT topic: UIPUT/out/{ws}/{dam}/{pic}/{de}/{sw}/{dam_model}/result

   {
     "version": "mt.v0",
     "op_id": "reg_ack_1770535046850",
     "records": [{
       "op": "add_label", "model_id": <DAM_MODEL_ID>,
       "p": 0, "r": 0, "c": 0,
...
```
1. 滑动 UI 通过 PIN 发送查询:
   { "action": "list_assets", "data": { "owner_model_id": 1001, "mime_prefix": "image/" } }
