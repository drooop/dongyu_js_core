...
- In scope:
  ...
  - 追加本地优先实施：本地 UI Server 使用远端 ZITADEL SSO，OIDC client id 固定为 `375920990745592038`，在 `localhost:30900` 下完成登录入口与 ToDo request/response timing trace 验证。
  - 增加 ToDo / bus_event_v2 的端到端轻量 trace：同一 `op_id` 必须至少能看到浏览器 dispatch、server 收包、server response、MQTT publish、MQTT response materialization、snapshot broadcast 的时间点。
- Out of scope:
  ...
  - 不改变 OIDC 远端租户、项目或用户；本地只消费既有远端 SSO app 配置。
...
- 本地 `http://localhost:30900` 可使用远端 SSO 配置启动，`/auth/sso/start` 使用 client id `375920990745592038`，callback 为 `http://localhost:30900/auth/sso/callback`。
- 本地或脚本验证能输出同一 `op_id` 的 timing breakdown，且不会让浏览器或外部 MQTT 直接写业务 label。
- 每个新增小阶段完成后进行 sub-agent code review；review 通过或修正后才能进入下一小阶段。
