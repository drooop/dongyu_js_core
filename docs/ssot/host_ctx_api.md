---
title: "Host Capability Interface (ctx API)"
doc_type: ssot
status: active
updated: 2026-03-21
source: ai
...
- `ctx.sendMatrix(payload)`
  ...
  - 仅在 `runtime_mode=running` 时允许执行；否则必须返回 `runtime_not_running`

## 3. PIN/MQTT 相关

- `ctx.startMqttLoop()`
  - 使用 Model 0 `(0,0,0)` 上的 `mqtt.local.ip` / `mqtt.local.port` labels 启动 MQTT loop
  - `mqtt.local.*` / `matrix.*` labels 仅在 Model 0 `(0,0,0)` 生效
  - trusted bootstrap 允许在 `boot/edit` 直写这些 labels，但真正 loop/adapter 生效必须等到 `running`

- `ctx.currentTopic(pinName)`
  - 计算 pin topic（Backward compatible）
  - 默认（Stage2）：依据 Model 0 上的 topic labels 计算
