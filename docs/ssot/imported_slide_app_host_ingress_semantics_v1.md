...
- 0424 target 将 imported slide App 安装实例从“分配一个全局 positive model id”升级为“分配一个 App instance `table_id`，包内 non-negative model ids 保持局部”。该目标合同见 `docs/ssot/principal_scoped_subtable_namespace_v1.md`，尚未由本 v1 current implementation 完成。
...
- 不允许在 zip 内声明运行时 `reply_to`、`route.reply_to` 或 `reply_target_*`。`reply_target_worker_id` / `reply_target_table_id` / `reply_target_model_id` / `reply_target_pin` 是宿主 UI Server 在发出消息时根据当前本地安装 App 的 table-qualified `ModelRef` 与 host identity 生成的 server-owned payload records。
...
```json
{
  ...
  "v": {
    ...
    "target": {
      ...
      "response_topic": "UIPUT/ws/dam/pic/de/U1/1050/result"
    },
    "reply_pin": "result",
...
- `target`：根据 `remote_bus_endpoint_v1`、当前 `from_pin`、`reply_pin` 和 Model 0 `mqtt_topic_base` 合成的远端 endpoint 与 response endpoint。`topic` 是 request 转发 truth；`response_topic` 是 response 转发 truth；`route_kind` 只决定 UI Server 先走控制总线还是管理总线。
...
0424 target 下，步骤中的 `origin_model_id` / `reply_target_model_id` 必须增加 table 维度，形成 `origin_table_id` / `reply_target_table_id`。否则多个 principal 或多个 installed App instance 中相同的 local `model_id` 会发生回包目标歧义。
