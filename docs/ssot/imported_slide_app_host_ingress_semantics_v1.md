...
- 同工作区正式业务 ingress 默认经 `bus_event_v2 -> Model 0 (0,0,0) pin.bus.cb.in`
...
- 在 root (0,0,0) 上声明 `remote_bus_endpoint_v1: json`，说明远端 provider worker / model 默认目标和 UI Server 出站路线。该声明只含 `transport`、可选 `route_kind`、远端 endpoint 的 `worker_id` 与 `model_id`，不含 `endpoint_pin`、`reply_target_*` 或旧 `route.reply_to`。
- 在 root (0,0,0) 声明一个或多个 `pin.out` 作为对外出口，并在 `dual_bus_model.egress_pins` 中逐一列出。0375 后 outbound packet 的 `endpoint_pin` 必须来自当前被触发的公开出口 pin，例如 `submit1`；不得再把 imported egress 固定为单一 `submit`。
...
- 不允许在 zip 内声明运行时 `reply_to`、`route.reply_to` 或 `reply_target_*`。`reply_target_worker_id` / `reply_target_model_id` / `reply_target_pin` 是宿主 UI Server 在发出消息时根据当前本地安装模型 id 与 host identity 生成的 server-owned payload records。
- 不允许在 `remote_bus_endpoint_v1` 内声明 `to.pin`；公开 pin 来源只能是 `dual_bus_model.egress_pins` 与当前触发的 root `pin.out`。
- imported slide app 的同工作区 UI egress 默认使用 `pin.bus.cb.out`。当 `remote_bus_endpoint_v1.route_kind="management"` 时，安装器必须使用 `pin.bus.mb.out`，形成 `UI Server -> management bus -> MBR -> control bus / MQTT -> Remote Worker` 的路径。
...
安装器在分配本地模型 id、确认当前 worker identity、确认远端 endpoint 和 `route_kind` 后，才可以在 imported root `(0,0,0)` 写入类似记录：
...
- `bus`：本次 egress 使用的总线类别；UI/滑动 App 用户交互默认是 `control`。
- `host_model_id` / `host_cell` / `host_pin_type` / `host_pin_key`：宿主实际接到的系统总线出口。
- `target`：根据 `remote_bus_endpoint_v1`、当前 `from_pin` 和 Model 0 `mqtt_topic_base` 合成的远端 endpoint。`topic` 是 MBR 转发 truth；`route_kind` 只决定 UI Server 先走控制总线还是管理总线。
- `reply_pin`：回包进入本地 imported app 时使用的公开回程 pin。
- `owned_by`：必须标明由安装器持有，provider ZIP 不得覆盖。

UI 可把 `ui.egress.binding.v1` 投影出来，让用户看到“这个按钮/出口通过哪个宿主总线 pin 外发”。但 authority 仍然来自实际 pin route；如果 binding 记录存在而实际 `pin.connect.*` / bus pin 不存在，必须视为安装不完整，而不是允许 UI 直接发送。
...
4. 同一个 bridge 写入 `message_role=request`、`origin_worker_id` / `origin_model_id` / `origin_pin`，并生成 server-owned `reply_target_worker_id` / `reply_target_model_id` / `reply_target_pin`，指向当前 UI Server / local installed model id / `result` pin；ZIP 内容不能覆盖它。
5. bridge 把 `bus_send.v1` Temporary ModelTable records 写入 `mt_bus_send_in`，其中包含 `message_role`、`bus_out_key`、`topic`、`bus`、`route_kind`、endpoint records、origin records、reply target records 与嵌套业务 `payload`。
...
8. MBR 只按 packet payload 中的 `topic` record 做通用转发，不要求 per-app route registration；management-routed 请求到达 MBR 后转为控制总线 / MQTT 发给 Remote Worker。回包继续使用同一个 endpoint topic，不生成 result topic。

### 9.3a remote-worker 内部公开 pin 到程序模型的接线

`endpoint_pin` 表示远端 provider model root 的公开 `pin.in`，不是直接调用程序模型。

如果 R1 的 Model `3000` 公开 `submit1` 并希望触发 `(1,1,1)` 的 `func.js submit1`，R1 应在模型内部这样填表：
