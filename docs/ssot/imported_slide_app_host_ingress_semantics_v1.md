...
- 不允许在 zip 内声明 `pin.bus.in` / `pin.bus.out` / `pin.bus.cb.*` / `pin.bus.mb.*` / `pin.connect.model` / `func.python`，这些由 `SLIDE_IMPORT_FORBIDDEN_LABEL_TYPES` 拒绝，防止 imported app 自建第二个对外入口。
...
### 9.2 宿主自动补齐的 egress adapter（0326 current truth, 0363 target annotated）
...
| Model 0 `(0,0,0)` | `imported_<semantic>_<id>_bus` `pin.bus.out` | 当前 0364 前宿主 egress bus-out key |
| Model 0 `(0,0,0)` | `imported_<semantic>_<id>_bus` `pin.bus.mb.out` | 0363 目标合同：UI/管理类 egress 应写入 DEM 管理总线出口 |
| imported root `(0,0,0)` | `ui_egress_<semantic>_binding_<id>` `ui.egress.binding.v1` | 0363 目标合同：host-owned 绑定说明，供 UI 投影显示该公开出口实际使用哪个宿主管理总线 pin |
| imported root `(0,0,0)` | `host_egress_generated_model0_labels` / `host_egress_generated_mount` | 删除清理清单 |

0364 前 live code 仍使用未拆分的 `pin.bus.out`。0364 实施时必须把 UI/管理类 egress 的宿主出口迁到 `pin.bus.mb.out`，并同步更新 importer、runtime、系统模型与部署补丁。

### 9.2a Host-owned UI egress binding（0363 target）
...
6. `mt_bus_send` 构造带 `route` 的 `pin_payload` packet，写入 `imported_<semantic>_<id>_bus`。0364 前该 key 的 type 是 `pin.bus.out`；0363 目标合同中 UI/管理类 egress 的 type 是 `pin.bus.mb.out`。
