...
4. 同一个 bridge 写入 `origin_worker_id` / `origin_model_id` / `origin_pin`，并生成 server-owned `reply_target_worker_id` / `reply_target_model_id` / `reply_target_pin`，指向当前 UI Server / local installed model id / `result` pin；ZIP 内容不能覆盖它。
5. bridge 把 `bus_send.v1` Temporary ModelTable records 写入 `mt_bus_send_in`，其中包含 `bus_out_key`、endpoint records、origin records、reply target records 与嵌套业务 `payload`。
...
8. `ProgramModelEngine` 在 intercept 后补扫 worker root 系统总线出口，按 packet `op_id` 做一次性 Matrix bridge。MBR 只按 packet payload 中的 endpoint records 做通用转发，不要求 per-app route registration。
