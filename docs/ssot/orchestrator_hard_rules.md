...
Authority:
- Below `CLAUDE.md`, `docs/WORKFLOW.md`, and `docs/ssot/execution_governance_ultrawork_doit.md`.
- This file only governs doit-auto / orchestrator state-machine behavior.

Scope:
- Batch state, iteration state, review/execution phase transitions, crash idempotency, and terminal summaries for orchestrated waves.
- It does not define runtime semantics, ModelTable contracts, or user-guide truth.

Conflict behavior:
- If this file conflicts with higher workflow or execution governance, stop and update the lower layer through an iteration.
- Tool names in this file describe implementation adapters for the orchestrator; if actual tooling changes, state transitions and evidence rules remain binding.
...
| 生成 plan/resolution、执行实现 | doit | configured execution agent |
| 评审 plan、review 执行结果 | ultrawork / reviewer | configured review agent or sub-agent |

禁止：
- orchestrator 不得自行评审或实现。
- review 阶段：只读 + 可跑验证；除非当前 iteration 明确授权，不可编辑文件。
- execution 阶段：可按已批准的 resolution 读写文件，并必须把验证证据写入 runlog。
...
此要求包含在 configured review prompt 中，不由 orchestrator 自行判断。

---

## §13 Driver 权限配置

### §13.1 Review driver（ultrawork / reviewer 角色）
...
driver 配置可以随当前可用工具调整，但必须保持只读 review 语义和可审计输出。

### §13.2 Execution driver（doit 角色）

Planning 阶段：只能写 iteration 计划、resolution、runlog 和必要索引；不得实现。

Execution 阶段：只在 review gate 通过后执行 approved resolution，并记录验证证据。

### §13.3 输出捕获

- Execution driver 输出最终消息与事件流到 transcript。
- Review driver 输出结构化 review verdict 到 transcript。
...
3. review driver 或 execution driver 调用失败（timeout / crash）连续 2 次。
