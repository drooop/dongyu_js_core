...
- `topic`，完整控制总线 MQTT topic，例如 `UIPUT/<ws_id>/<dam_id>/<pic_id>/<de_id>/<sw_id>/<worker_id>/<model_id>/<pin>`
- `route_kind`，可选；缺省等同 `"control"`；显式 `"management"` 表示本次消息需要先走管理总线到 MBR，再由 MBR 按 `topic` 转为目标控制总线
- endpoint metadata records：`endpoint_worker_id` / `endpoint_model_id` / `endpoint_pin`
- origin metadata records：`origin_worker_id` / `origin_model_id` / `origin_pin`
- reply target metadata records：`reply_target_worker_id` / `reply_target_model_id` / `reply_target_pin`
...
`message_role="request"` 表示该 payload 会触发目标 worker/model/pin 上的程序链路；`message_role="response"` 表示这是回包，只能按相同 topic 返回并由 UI Server 按 `reply_target_*` materialize。远端 runtime 收到自己 endpoint topic 上的 `response` 必须忽略，不能再次触发程序。

任何外发 payload 缺少或非法 `topic` record 时，MBR / MQTT adapter 必须 fail closed，并写可观察错误。endpoint records 仍是远端 worker 内部公开 pin 分发和审计信息，不是 MBR 的路由 truth。任何需要回写本地 UI Server / imported app 的请求或回包，若缺少或非法 reply target records，必须 fail closed；不得从 topic 末段、`pin=result`、`route.reply_to`、`return_topic`、`returnTopic`、`result_topic` 或旧 result topic 推断回写目标。
...
该 topic 必须正好 9 段。所有段必须非空；`model_id` 必须是正整数；`worker_id` 与 `pin` 不能包含 `/`、`+`、`#`。旧格式 `UIPUT/.../worker/<worker_id>/model/<model_id>/pin/<pin>`、旧 `<base>/<model_id>/<pin>`、缺段或增段都不是合法输入面，必须 fail closed，不能兼容解析。
...
请求和回包使用同一个 endpoint topic；MBR 不得把 `reply_target_*` 派生成 result topic。
...
- `origin_worker_id` / `origin_model_id` / `origin_pin`：由 UI Server 根据当前 host identity、本地安装模型 id 与触发 pin 生成
- server-owned `reply_target_worker_id` / `reply_target_model_id` / `reply_target_pin`：由 UI Server 根据当前 host identity、本地安装模型 id 与本地接收 pin 生成
