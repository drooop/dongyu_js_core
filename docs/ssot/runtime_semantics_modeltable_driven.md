...
- `response_topic`，request 期待的回包 MQTT topic，由 `reply_target_*` 派生，例如 `UIPUT/<ws_id>/<dam_id>/<pic_id>/<de_id>/<reply_target_worker_id>/<reply_target_model_id>/<reply_target_pin>`
...
任何外发 payload 缺少或非法 `topic` record 时，MBR / MQTT adapter 必须 fail closed，并写可观察错误。需要回包的 request 若缺少、非法或同于 `topic` 的 `response_topic`，必须 fail closed。response 若 `topic !== response_topic`，或 `endpoint_*` 与 `reply_target_*` 不一致，也必须 fail closed。endpoint records 描述当前 packet 的投递 endpoint，不是 MBR 的路由 truth。任何需要回写本地 UI Server / imported app 的请求或回包，若缺少或非法 reply target records，必须 fail closed；不得从 topic 末段、`pin=result`、`route.reply_to`、`return_topic`、`returnTopic`、`result_topic` 或旧 result topic 推断回写目标。
...
- `response_topic`：由 `mqtt_topic_base`、`reply_target_worker_id`、`reply_target_model_id` 与 `reply_target_pin` 合成；request 写入该值，response 的 `topic` 必须改为该值
