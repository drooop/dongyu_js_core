...
`message_role="request"` 表示该 payload 会触发目标 worker/model/pin 上的程序链路；`message_role="response"` 表示这是回包，只能被转回管理总线并由 UI Server 按 `reply_target_*` materialize。远端 runtime 收到自己 endpoint topic 上的 `response` 必须忽略，不能再次触发程序。

任何外发 payload 缺少或非法 endpoint records 时必须 fail closed，并写可观察错误。任何需要回写本地 UI Server / imported app 的请求或回包，若缺少或非法 reply target records，必须 fail closed；不得从 topic 末段、`pin=result`、`route.reply_to`、`return_topic`、`returnTopic`、`result_topic` 或旧 result topic 推断回写目标。

### 7.2 Routing

MBR 不维护每个滑动 App 的静态 route truth。转发目标由消息内 endpoint metadata records 决定。唯一合法 MQTT endpoint topic 格式是：
...
管理总线转控制总线只转发 `message_role="request"`；控制总线回管理总线只转发 `message_role="response"`。请求和回包使用同一个 endpoint topic；MBR 不得把 `reply_target_*` 派生成 result topic。
...
该声明只表达远端 provider 默认目标。运行时出站 packet 必须合成 Temporary ModelTable record array metadata：
