...
- `response_topic` 必须由 UI Server / 宿主在发出 request 时生成，形态为 `<mqtt_topic_base>/<reply_target_worker_id>/<reply_target_model_id>/<reply_target_pin>`。request 的 `response_topic` 必须合法且不得等于 `topic`；response 的 `topic` 必须等于 `response_topic`。
...
- 请求使用远端 endpoint topic；回包使用 `response_topic`，且 response packet 的 `endpoint_*` 必须与 `reply_target_*` 一致。
