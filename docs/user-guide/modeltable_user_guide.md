...
该 topic 必须正好 8 段。每个动态分段必须是非空安全字符串，不能包含 `/`、`+`、`#`；`model_id` 必须是正整数。含冗余 software-worker 段的旧 topic、旧的 `UIPUT/.../worker/<worker_id>/model/<model_id>/pin/<pin>`、旧的 `UIPUT/.../<model_id>/<pin>`、return topic、result topic 都不是当前输入面。
...
返回 payload 必须把 `message_role` 写成 `response`。`topic` 必须等于 request 中的 `response_topic`，例如 `UIPUT/ws/dam/pic/de/U1/1055/result`。response packet 的 `endpoint_*` 必须描述当前投递 endpoint，因此要与 `reply_target_*` 一致；远端处理者身份写在 `origin_*` 中。
