...
- `0362` 新增 imported slide app 的自描述远端路由：
  ...
  - UI Server 身份、本地安装 App 的 `table_id + model_id` 与回写目标必须由 UI Server 运行时写成 Temporary ModelTable payload records，ZIP 不能提供或覆盖
...
[
  ...
  { "id": 0, "p": 0, "r": 0, "c": 0, "k": "origin_table_id", "t": "str", "v": "app:demo:user-a:submit" },
...
如果 `topic`、endpoint metadata 或 reply target metadata 缺失或非法，MBR 必须拒绝该消息并写入可观察错误。MBR 的转发目标只来自 payload `topic` record，endpoint metadata 只用于远端 worker 内部分发和审计。不能把普通业务 JSON 当成 fallback 直接发送。不能把 `origin_*` / `reply_target_*` 写成外层 JSON 字段；它们必须是 record array 中的 records。对安装后的滑动 App，`origin_model_id` / `reply_target_model_id` 必须和 `origin_table_id` / `reply_target_table_id` 一起出现，形成 table-qualified `ModelRef`。
