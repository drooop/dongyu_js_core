...
- `0362` 新增 imported slide app 的自描述远端路由：
  ...
  - 它只声明远端 endpoint 的 `worker_id` 与 `model_id`
  - `endpoint_pin` 来自当前公开入口 pin，例如 `submit1`
  - UI Server 身份、本地安装模型 id 与回写目标必须由 UI Server 运行时写成 Temporary ModelTable payload records，ZIP 不能提供或覆盖
...
MBR 转发时只使用消息 payload records 中的 `topic` record 决定目标，不需要为每个 App 预注册静态 route。`endpoint_*` records 不参与 MBR topic 选择，只用于 remote-worker 收到消息后的内部公开 pin 分发。

```text
UIPUT/<ws_id>/<dam_id>/<pic_id>/<de_id>/<sw_id>/<worker_id>/<model_id>/<pin>
```

该 topic 必须正好 9 段。每个动态分段必须是非空安全字符串，不能包含 `/`、`+`、`#`；`model_id` 必须是正整数。旧的 `UIPUT/.../worker/<worker_id>/model/<model_id>/pin/<pin>`、旧的 `UIPUT/.../<model_id>/<pin>`、return topic、result topic 都不是当前输入面。
...
[
  ...
  { "id": 0, "p": 0, "r": 0, "c": 0, "k": "origin_model_id", "t": "int", "v": 1055 },
...
如果 `topic`、endpoint metadata 或 reply target metadata 缺失或非法，MBR 必须拒绝该消息并写入可观察错误。MBR 的转发目标只来自 payload `topic` record，endpoint metadata 只用于远端 worker 内部分发和审计。不能把普通业务 JSON 当成 fallback 直接发送。不能把 `origin_*` / `reply_target_*` 写成外层 JSON 字段；它们必须是 record array 中的 records。
...
### 6.4 Explicit Management Route

当 UI Server 不能直接访问目标 MQTT / 控制总线时，滑动 App 可以在 `remote_bus_endpoint_v1` 里显式写：
...
remote-worker 回包时，写入自身 Model 0 的 `pin.bus.cb.out`。MBR 收到后仍按 payload `topic` record 转发；UI Server 收包后只能按 `reply_target_*` / `origin_*` records 选择本地 owner materialization 目标，再写入目标 UI 模型 label。
...
在 UI Server 所在 DEM 的 Model 0 `(0,0,0)` 写入一个 `pin.bus.cb.out` label。`v` 是临时 ModelTable records，其中 `topic` 是 MBR 的唯一转发目标，`endpoint_*` 指向 remote-worker 的公开 pin，`origin_*` / `reply_target_*` 指向 UI Server 本地安装实例：
...
- MBR 按 payload `topic` record 转发 remote-worker 的控制总线回包；默认仍是控制总线消息。
