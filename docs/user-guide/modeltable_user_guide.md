...
MBR 转发时使用消息 payload records 中的 endpoint metadata 决定目标，不需要为每个 App 预注册静态 route。
...
如果 endpoint metadata 缺失或非法，MBR 必须拒绝该消息并写入可观察错误。不能把普通业务 JSON 当成 fallback 直接发送。不能把 `origin_*` / `reply_target_*` 写成外层 JSON 字段；它们必须是 record array 中的 records。

### 6.3 Management To Control

UI Server 要把业务请求发给 remote-worker 时，写入本机 DEM Model 0 的 `pin.bus.mb.out`。MBR 收到后读取 `endpoint_*` records，再把消息转成目标 worker 的控制总线 topic。
...
MBR 也支持管理总线消息转管理总线消息。此时仍由 DEM Model 0 的 `pin.bus.mb.out` 承载，MBR 根据 `endpoint_worker_id`、`endpoint_model_id`、`endpoint_pin` records 发送到另一个 DEM 的管理总线入口。

这条路径同样只认 `pin_payload.v1` 临时 ModelTable records，不接受旧 envelope 或普通 JSON fallback。

### 6.5 Control To Management Return

remote-worker 回包时，写入自身 Model 0 的 `pin.bus.cb.out`。MBR 收到后仍按 endpoint topic 转发；UI Server 收包后只能按 `reply_target_*` / `origin_*` records 选择本地 owner materialization 目标，再写入目标 UI 模型 label。
...
在 UI Server 所在 DEM 的 Model 0 `(0,0,0)` 写入一个 `pin.bus.mb.out` label。`v` 是临时 ModelTable records，其中 `endpoint_*` 指向 remote-worker 的公开 pin，`origin_*` / `reply_target_*` 指向 UI Server 本地安装实例：
...
- MBR 根据 `endpoint_*` records 发布到 remote-worker 的控制总线 topic。
