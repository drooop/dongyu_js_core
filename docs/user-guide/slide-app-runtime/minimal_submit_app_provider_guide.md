...
当前基准文件是 `test_files/minimal_submit_dual_bus_app_payload.json`。它是一个 61 条 record 的 ModelTable records array。对应 ZIP 是 `test_files/minimal_submit_dual_bus.zip`，内部只有一个文件：`app_payload.json`。
...
| `click_chain` | `pin.out` | Submit 按钮点击后写入的 Cell 出口。 |
...
按钮 Cell 至少要有。注意这里的字段名是 `write.pin`，不是普通对象里的任意 `pin`：
...
```text
ui_bind_json writes modeltable value_ref to click_chain
...
|---|---|---|---|---|
| Submit | `click_chain` | `submit_request` | `handle_submit` | `submit1` |
| Approve | `approve_click_chain` | `approve_request` | `handle_approve` | `approve1` |
...
导出过滤规则会排除 `host_ingress_generated_*`、`host_egress_generated_*`、`deletable`、`installed_at`、`imported_bundle_model_ids`、`import_root_temp_id` 等安装态 labels。
