...
程序模型 `submit1` 的内容可以从最小回显开始。真实环境里它还需要按 `route.reply_to` 交给 RE 的总线出口；这属于 RE 自己的 Model 0 之后内部实现，不写进 UI Server zip。
...
| `(2,4,0)` | Submit 按钮 | `bus_event_v2=true`，入口由本地安装模型 id 与公开 pin 派生 |
