...
-> reply_target_table_id + reply_target_model_id owner materialization
-> UI 组件读取更新后的 labels
```
...
| `response_topic` | remote-worker 必须把 response 发回哪里，例如 `UIPUT/ws/dam/pic/de/U1/1087/result`。这里是 host transport endpoint。 |
...
这里的 `1087` 只是 host transport endpoint 示例，不是 App table 内的 root model。真实 materialization 目标要从 request 的 `reply_target_table_id` 和 `reply_target_model_id` 一起读取，不要写死。
...
remote-worker 程序模型收到 request 后，应该从 request records 读取 `response_topic` 和 `reply_target_*`。`response_topic` 用来解析本次回包投递到 UI Server 的 host transport endpoint；`reply_target_*` 原样带回，用来让 UI Server 找到最终要写入的 App table。
...
const replyTarget = {
  ...
  table_id: readPayload('reply_target_table_id'),
