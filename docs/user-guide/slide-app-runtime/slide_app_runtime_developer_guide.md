...
不要再显式声明 `handle_submit:in` / `handle_submit:out` / `handle_submit:logout` 为普通 pin。它们是函数 `handle_submit` 自动拥有的端口。若导入包里已经出现这类重复声明，按 `function_port_collision_repair_guide.md` 修正。
