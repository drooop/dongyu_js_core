...
3. 宿主的 `model0_route` 把 value 再转到 Model 0 `(0,0,0)` 的 `__host_egress_submit_bridge_in_<id>`。
4. `bridge_imported_submit_to_mt_bus_send_<id>` 把这次写入转成 `mt_bus_send_in` 请求。
5. `mt_bus_send` 构造 `pin_payload v1` packet，写入 Model 0 `pin.bus.out`。
6. runtime 对该 `pin.bus.out` 做 MQTT publish，`ProgramModelEngine` 再按 packet `op_id` 做一次性 Matrix bridge。

卸载 imported app 时，这一整套 labels（ingress + egress bridge + bus-out key）都按 `host_ingress_generated_*` / `host_egress_generated_*` 清单 rm_label 清理。
