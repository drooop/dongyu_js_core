...
它的职责是把 app root 的 `pin.out` 接到宿主外发链，再由 Model 0 统一转成 `pin.bus.mb.out`。没有外发需求的 app 不需要这一层。
...
- server 只把合法 envelope 写入 Model 0 的 `pin.bus.mb.in`。
...
6. runtime 写入 Model 0 `pin.bus.mb.out`。
