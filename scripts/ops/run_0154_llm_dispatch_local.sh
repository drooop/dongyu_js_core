...
usage() {
  cat <<'EOF'
Usage:
  scripts/ops/run_0154_llm_dispatch_local.sh [options]

Options:
  --port <port>         UI server port (default: 9012)
  --llm-port <port>     Mock ollama port (default: 11435)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "[run-0154] unknown option: $1" >&2
      usage
      exit 1
      ;;
  esac
...
need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "[run-0154] missing command: $1" >&2
    exit 1
  fi
}
  ...
  if [ "$STOP_AFTER" -eq 1 ]; then
    if [ -f "$SERVER_PID_FILE" ]; then
      ...
      if [ -n "${spid:-}" ] && kill -0 "$spid" >/dev/null 2>&1; then
        ...
        echo "[run-0154] stopped ui-server pid=$spid"
      fi
    fi
    if [ -n "${MOCK_PID:-}" ] && kill -0 "$MOCK_PID" >/dev/null 2>&1; then
      kill "$MOCK_PID" >/dev/null 2>&1 || true
      echo "[run-0154] stopped mock-ollama pid=$MOCK_PID"
    fi
  fi
}
trap cleanup EXIT
...
echo "[run-0154] step1 start local ui-server"
START_ARGS=(
  --port "$PORT"
...
echo "[run-0154] step2 verify llm dispatch path"
bash "$SCRIPT_DIR/verify_llm_dispatch_roundtrip.sh" --base-url "http://127.0.0.1:${PORT}"
