...
function test_server_default_prompt_mentions_new_pin_types() {
  ...
  assert.match(text, /model_id=0/, 'server template must constrain model_id=0 bus usage');
  assert.match(text, /model_id != 0|model_id!=0/, 'server template must constrain non-zero model boundary usage');
