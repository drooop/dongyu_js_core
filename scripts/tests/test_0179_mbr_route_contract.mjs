...
assert.equal(published.payload?.version, 'mt.v0', 'route-driven bridge payload must still be mt.v0');
assert.equal(published.payload?.op_id, 'route_101_001', 'route-driven bridge must preserve op_id');
assert.deepEqual(
  published.payload?.records?.map((record) => record.model_id),
  [101, 101, 101],
  'route-driven bridge must target the source model instead of hardcoded Model 100',
);
