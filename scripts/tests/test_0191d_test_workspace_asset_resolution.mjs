...
function test_test_page_prefers_model_asset() {
  ...
  store.consumeOnce();
  const result = resolvePageAsset(store.snapshot, { pageName: 'test', projectCellwiseModel: buildAstFromCellwiseModel });
...
function test_workspace_page_prefers_model_asset() {
  ...
  store.consumeOnce();
  const result = resolvePageAsset(store.snapshot, { pageName: 'workspace', projectCellwiseModel: buildAstFromCellwiseModel });
  assert.equal(result.source, 'model_asset');
  assert.equal(result.assetType, 'cellwise_model');
