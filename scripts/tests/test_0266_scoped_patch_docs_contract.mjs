...
function test_runtime_semantics_freezes_scoped_patch_rules() {
  ...
  assert.match(text, /mt_write|owner materializer/u, 'runtime semantics must define canonical owner materialization path');
...
function test_host_ctx_api_removes_runtime_wide_patch_surface() {
  ...
  assert.match(text, /mt_write_req|owner materializer/u, 'ctx api must describe explicit materialization path');
...
function test_label_registry_freezes_helper_contract() {
  ...
  assert.match(text, /mt_write.*mt_bus_receive.*mt_bus_send/u, 'label registry must mention root default program contract');
  assert.match(text, /helper_executor.*不再由 runtime 自动种入/u, 'label registry must mark helper executor as removed');
...
function test_user_guide_mentions_helper_only_materialization() {
  ...
  assert.match(text, /mt_write|owner materializer/u, 'user guide must explain explicit materialization');
