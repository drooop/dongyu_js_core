...
const templatePath = path.join(repoRoot, 'packages/worker-base/system-models/templates/data_queue_v0.json');
...
async function buildRuntimeFromTemplate() {
  ...
  assert.equal(result.rejected, 0, 'queue template patch should apply without rejections');
  runtime.setRuntimeMode('edit');
  runtime.setRuntimeMode('running');
  const model = runtime.getModel(2101);
  assert.ok(model, 'Data.Queue template must create model 2101');
...
async function buildRuntimeFromRemappedTemplate(modelId) {
  const runtime = new ModelTableRuntime();
  const patch = remapPatchModelId(loadJson(templatePath), 2101, modelId);
  const result = runtime.applyPatch(patch, { allowCreateModel: true, trustedBootstrap: true });
  assert.equal(result.rejected, 0, 'remapped queue template patch should apply without rejections');
  runtime.setRuntimeMode('edit');
  runtime.setRuntimeMode('running');
  const model = runtime.getModel(modelId);
  assert.ok(model, `Data.Queue template must create remapped model ${modelId}`);
  return { runtime, model };
}

function findCase(name) {
  const fixture = loadJson(fixturePath).queue[name];
  assert.ok(fixture, `missing queue fixture case ${name}`);
  return fixture;
}
...
async function test_enqueue_dequeue_fifo_contract() {
  const { runtime, model } = await buildRuntimeFromTemplate();
  const fixture = findCase('enqueue_dequeue_fifo');
  ...
  assert.deepEqual(runtime.getLabelValue(model, 0, 0, 0, 'dequeue_data_out'), fixture.expected.dequeue_data_out);
...
async function test_queue_template_can_materialize_to_non_2101_model_id() {
  const { runtime, model } = await buildRuntimeFromRemappedTemplate(29161);
  runtime.addLabel(model, 0, 0, 0, {
    k: 'enqueue_data_in',
    t: 'pin.in',
    v: [
      { id: 0, p: 0, r: 0, c: 0, k: 'model_type', t: 'model.single', v: 'Data.Single' },
      { id: 0, p: 0, r: 0, c: 0, k: 'value', t: 'json', v: 'Q' },
    ],
  });
  await settle();
  assert.equal(runtime.getLabelValue(model, 0, 1, 0, 'value'), 'Q', 'remapped queue template must still write into remapped model');
  assert.equal(runtime.getLabelValue(model, 0, 0, 0, 'size_now'), 1, 'remapped queue template must update size_now');
}

const tests = [test_enqueue_dequeue_fifo_contract, test_queue_template_can_materialize_to_non_2101_model_id];
