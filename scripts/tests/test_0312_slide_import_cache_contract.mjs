...
async function test_slide_importer_repairs_legacy_persisted_click_contract() {
  return withServerState(async (server, state) => {
    ...
    assert.equal(readLabelFromState(state, 0, 2, 0, 13, 'model_type')?.t, 'model.submtconnection', 'repair_must_restore_importer_model0_mount');
    assert.equal(readLabelFromState(state, 0, 1, 0, 3, 'model_type')?.t, 'model.submtconnection', 'repair_must_restore_system_model0_mount');
