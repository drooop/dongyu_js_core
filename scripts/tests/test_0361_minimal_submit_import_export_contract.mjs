...
async function test_saved_zip_imports_and_exports_reimportable_zip() {
  return withServerState(async (state, buildSlideAppExportZip) => {
    ...
    assert.equal(exportedPayload.some((record) => record.k === 'dual_bus_model' && record.v?.mode === 'imported_host_egress'), true, 'exported_payload_must_keep_dual_bus_declaration');
    assert.equal(exportedPayload.some((record) => record.k === 'submit' && record.t === 'pin.out'), true, 'exported_payload_must_keep_submit_pin_out');
