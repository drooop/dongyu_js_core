...
function test_mbr_does_not_echo_own_route_to_mqtt_publish() {
  ...
  assert.equal(rt.getCell(sys, 0, 0, 0).labels.get('change_out'), undefined, 'MBR must not bridge its own route.to MQTT publish back to Matrix');
...
async function test_remote_worker_submit1_receives_route_and_replies_to_reply_to() {
  ...
  rt.mqttClient = {
    ...
    subscribe(topic) { this.subscriptions.add(topic); },
  ...
  assert.equal(root.get('result')?.v?.find((record) => record.k === 'display_text')?.v, 'Submitted: browser submit', 'remote submit handler must emit provider result pin payload');
...
function test_mbr_mqtt_inbound_rejects_invalid_temporary_modeltable_records() {
  ...
  assert.equal(rt.getCell(sys, 0, 0, 0).labels.get('change_out'), undefined, 'invalid MQTT inbound payload must not be bridged to MGMT_OUT');
  assert.equal(
