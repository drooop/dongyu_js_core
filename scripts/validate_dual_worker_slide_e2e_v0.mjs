...
async function main() {
  if (!isEnabled()) {
    ...
    console.log('dual_worker_slide_e2e_v0: SKIP (disabled by default)');
  ...
  try {
  // Prefer token auth; otherwise login once to avoid Synapse rate limits.
  const baseUrl = String(process.env.MATRIX_HOMESERVER_URL || '');
  const { accessToken, userId } = await ensureMatrixAuth(baseUrl);

  // Ensure further Matrix adapters use token auth (avoid extra logins).
  ...
  const baseEnv = {
    ...
    // Use token auth to avoid login rate limit.
  ...
  // Wait for remote and UI HTTP to respond.
  ...
  const opId = `op_slide_${Date.now()}`;
  const patch = {
    version: 'mt.v0',
    op_id: opId,
    records: [
      { op: 'add_label', model_id: remoteModelId, p: 0, r: 0, c: 0, k: 'slide_demo_text', t: 'str', v: 'hello' },
    ],
  };
  const evt = { version: 'v0', type: 'ui_event', op_id: opId, payload: patch };
    ...
    console.log('VALIDATION RESULTS');
    console.log(`dual_worker_slide_e2e_v0: PASS room_id=${roomId} op_id=${opId}`);
  } finally {
    cleanup();
  }
}
