...
function casePublishActionRemoved() {
  const rt = createRuntimeWithSystem();
  enqueueJob(rt, 1, 'mgmt_publish', { channel: 'change_out', payload: { hello: 1 } });
  runIntentDispatchOnce(rt);
  assertRemovedAction(rt, 1, 'mgmt_publish');
  return { key: 'mgmt_publish_removed', status: 'PASS' };
}

function caseBindActionRemoved() {
  const rt = createRuntimeWithSystem();
  enqueueJob(rt, 2, 'mgmt_bind_in', { channel: 'channel_alpha', target_ref: { model_id: 2, p: 0, r: 0, c: 0, k: 'inbox' } });
  runIntentDispatchOnce(rt);
...
try {
  ...
  const results = which === 'mgmt_publish' ? [casePublishActionRemoved()] : [casePublishActionRemoved(), caseBindActionRemoved()];
  output(results);
} catch (err) {
